...
async function sendNote(body) {
	try {
		...
		if (body["object_attributes"]["original_position"] != null) {
			...
			note.codeArea = await getLinesSecrionFromCode(
				note.codeArea,
				body["object_attributes"]["original_position"]["line_range"]["start"]["new_line"],
				body["object_attributes"]["original_position"]["line_range"]["end"]["new_line"]
			);
			if (note.codeArea != "") {
...
function getEmbed(note) {
	return new EmbedBuilder()
		...
		.setAuthor({
			...
			iconURL: note.creator["avatar_url"],
			url: note.creator["web_url"],
		})
		.setDescription(
			`${note.additiondalDescription}${note.description} 
        [Note link](${note.url})`
		)
		.addFields(
			{
				name: "Project",
				value: `[${note.project_name}](${note.project_url})`,
				inline: true,
			},
			{
				name: "Merge branch flow",
				value: `${note.data["merge_request"]["source_branch"]} -> ${note.data["merge_request"]["target_branch"]}`,
				inline: true,
			}
		)
		.setTimestamp();
...
async function getLinesSecrionFromCode(code, from, to) {
	...
	stringRows.forEach((str) => {
		if (str[0] !== " ") isNeedClearTabulation = false;
	});
	let newStringRows = [];
